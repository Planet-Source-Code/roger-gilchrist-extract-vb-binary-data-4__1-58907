...
Begin VB.Form frmExtractVB 
   ...
   Caption         =   "Extract images/text from VB binary support files"
   ClientHeight    =   8190
   ClientLeft      =   45
   ClientTop       =   630
   ClientWidth     =   10635
   ...
   Begin VB.CommandButton cmdDeleteText 
      Caption         =   "<Delete"
      Enabled         =   0   'False
      Height          =   255
      Left            =   4800
      ...
      Width           =   855
   End
   Begin VB.ListBox lstTextExtracted 
      Height          =   2595
      Left            =   0
      TabIndex        =   22
      Top             =   5280
   ...
   Begin VB.Frame fraExtractVB 
      ...
      Begin VB.PictureBox picCFXPBugFixfrmExtractVB 
         ...
         Top             =   175
         Width           =   5520
         Begin VB.CheckBox chkBinaryOnly 
            BackColor       =   &H8000000D&
            Caption         =   "Only Binary Text/Lists"
            Height          =   255
            Left            =   3480
            TabIndex        =   26
            Top             =   2160
      ...
      End
   End
   Begin VB.Label lblListTextCaption 
      Caption         =   "List/ItemData                        Text/Caption/Tag/Tooltip"
      Height          =   255
      Left            =   5760
      TabIndex        =   28
      Top             =   5040
...
Private Sub ClearForExtraction()
  ...
  lblExtractVB(0).Caption = "No images extracted"
  lblExtractVB(1).Caption = "No Text/Lists extracted"
  nTextCount = 0
  nImageCount = 0
  Erase PicData
  lstTextExtracted.Clear
  txtDisplay.Text = ""
  lstDisplay.Clear
...
Private Sub cmdDeleteText_Click()

  Dim OldPos As Long

  On Error Resume Next
  With lstTextExtracted
    If .ListIndex > -1 Then
      OldPos = .ListIndex
      Kill .List(.ListIndex)
      ...
      lstDisplay.Clear
      txtDisplay.Text = ""
      cmdDeleteText.Enabled = .ListCount
      If OldPos < .ListCount - 1 Then
        .ListIndex = OldPos
       ElseIf OldPos - 1 > -1 Then
        .ListIndex = OldPos - 1
       ElseIf .ListCount = 1 Then
        .ListIndex = 0
      End If
    End If
  End With 'lstTextExtracted
  On Error GoTo 0
...
Private Function ConvertMainFiletoXfile(varFile As Variant) As String

  '*.frm;*.dob;*.ctl;*.dsr;*pag

  Select Case LCase$(Right$(varFile, 4))
   Case ".frm"
    ConvertMainFiletoXfile = Left$(varFile, Len(varFile) - 3) & "frx"
   Case ".dob"
    ConvertMainFiletoXfile = Left$(varFile, Len(varFile) - 3) & "dox"
   Case ".ctl"
    ConvertMainFiletoXfile = Left$(varFile, Len(varFile) - 3) & "ctx"
   Case ".dsr"
    ConvertMainFiletoXfile = Left$(varFile, Len(varFile) - 3) & "dsx"
   Case ".pag"
    ConvertMainFiletoXfile = Left$(varFile, Len(varFile) - 3) & "pax"
  End Select
      ...
      For I = 0 To (nInCount - 1)
        ...
        For J = LBound(arrHidden) To UBound(arrHidden)
          ...
          If MatchString(sString, "BEGIN ") Then
            ...
            End If
            If InStr(sString, "MSComctlLib.ImageList") Or InStr(sString, "ComctlLib.ImageList") Then
           ...
           ElseIf IsFrxGraphicLine(sString, strControlProperty) Then
            ...
            If LenB(sImageData) And chkExtract(0).Value = vbChecked Then
            ...
            End If
           ElseIf IsFrxTextLine(sString, strControlProperty) And chkExtract(1).Value = vbChecked Then
            sImageData = GetStringData(sString, sFileIn(I))
            ProcessOneString sImageData, sFolder, strFormName, strControlName, strControlIndex, strControlProperty, nTextCount
           ElseIf IsFrxListLine(sString, strControlProperty) And chkExtract(2).Value = vbChecked Then
            sImageData = GetStringData(sString, sFileIn(I))
            ProcessOneString sImageData, sFolder, strFormName, strControlName, strControlIndex, strControlProperty, nTextCount
            Beep
          End If
...
Private Function ExtractList(ByVal sString As String, _
                             ByVal sSourceFile As String) As String
  ...
  If n Then
    ...
    If FileExist(sFile) Then
      ...
      '  For I = nOffset To nFileSize - 1
      '  If abFile(I) <> 0 Then
      '  ExtractList = ExtractList & Chr(abFile(I))
      '  Else
      '  If LenB(ExtractList) Then
      '  Found = Found + 1
      ...
      Do
        If abFile(I) <> 0 Then
          ...
          Next J
          Found = Found + 1
          '  ExtractList = ExtractList & Chr(0)
        End If
        I = J
      Loop While Found < Members
    End If
  End If
  ExtractList = Join(arrData, "|")

End Function

Private Function ExtractString(ByVal sString As String, _
                               ByVal sSourceFile As String) As String
  ...
  n = InStr(sString, ":")
  If n Then
    sFile = AttachPath(StripQuotes(Mid$(Left$(sString, n - 1), 2)), ExtractPath(sSourceFile))
    If FileExist(sFile) Then
      sString = "&H" & Trim$(Mid$(sString, n + 1))
      nOffset = CLng(sString) + 1 '+ PrevOffset
      FF = FreeFile
      ...
      For I = nOffset To nFileSize - 1
        If abFile(I) <> 0 Then
          ExtractString = ExtractString & Chr$(abFile(I))
         Else
          If LenB(ExtractString) Then
            Exit For
          End If
        End If
      Next I
    End If
  End If
  ExtractString = Left$(ExtractString, Len(ExtractString) - 1)
...
Private Sub GetHiddenTxt(ByVal strFilename As String, _
  ...
  'v4 faster extaction only collects the object names and binary data links
  strTarget = Mid$(strFilename, InStrRev(strFilename, "\") + 1)
  strTarget = ConvertMainFiletoXfile(strTarget)
  FN = FreeFile
  Open strFilename For Input Access Read Shared As FN
  Do
    Line Input #FN, strTemp
    If InStr(1, strTemp, strTarget, vbTextCompare) Then
      strData = strData & vbNewLine & Trim$(strTemp)
    End If
    If MatchString(Trim$(strTemp), "BEGIN ") Then
      strData = strData & vbNewLine & Trim$(strTemp)
    End If
    If MatchString(Trim$(strTemp), "INDEX ") Then
      strData = strData & vbNewLine & Trim$(strTemp)
    End If
    If MatchString(Trim$(strTemp), "BeginProperty ListImage") Then
      strData = strData & vbNewLine & Trim$(strTemp)
    End If
    'support for naming listimage items
    If MatchString(Trim$(strTemp), "Key ") Then
      strData = strData & vbNewLine & Trim$(strTemp)
    End If
    If MatchString(Trim$(strTemp), "Object.Tag ") Then
      strData = strData & vbNewLine & Trim$(strTemp)
    End If
    If MatchString(Trim$(strTemp), "Object.Tag ") Then
      strData = strData & vbNewLine & Trim$(strTemp)
    End If
    'for string literals rather than binary strings
    If InStr(strTemp, strTarget) = 0 Then
      If MatchString(Trim$(strTemp), "Tag ") Then
        strData = strData & vbNewLine & Trim$(strTemp)
      End If
      If MatchString(Trim$(strTemp), "Text ") Then
        strData = strData & vbNewLine & Trim$(strTemp)
      End If
      If MatchString(Trim$(strTemp), "ToolTipText ") Then
        strData = strData & vbNewLine & Trim$(strTemp)
      End If
      If MatchString(Trim$(strTemp), "Tag ") Then
        strData = strData & vbNewLine & Trim$(strTemp)
      End If
      If MatchString(Trim$(strTemp), "Caption ") Then
        strData = strData & vbNewLine & Trim$(strTemp)
      End If
      '    If MatchString(Trim$(strTemp), "TextRTF ") Then
      '      strData = strData & vbNewLine & Trim$(strTemp)
      '    End If
    End If
...
Private Function GetStringData(ByVal sString As String, _
  ...
  n = InStr(sString, "=")
  If n Then
    If InStr(sString, ConvertMainFiletoXfile(ExtractFileName(strFilename))) Then
      sString = Trim$(Mid$(sString, n + 1))
      If InStr(sString, "$") = 1 Then
        GetStringData = ExtractString(sString, strFilename)
       Else
        GetStringData = ExtractList(sString, strFilename)
      End If
     Else
      If chkBinaryOnly.Value = vbUnchecked Then
        GetStringData = Trim$(Mid$(sString, n + 1))
        GetStringData = Mid$(GetStringData, 2)
        GetStringData = Left$(GetStringData, Len(GetStringData) - 1)
      End If
    End If
  End If
...
Private Function IsFrxGraphicLine(ByVal sString As String, _
  ...
  strProp = Split(Trim$(sString))(0)
  'IsFrxTextLine = InStr("|Text|Caption|Tag|ToolTipText|TextRTF|", "|" & strProp & "|")
  IsFrxGraphicLine = InStr("|Icon|MouseIcon|DisabledPicture|DownPicture|Picture|MaskPicture|TooBoxBitMap|", "|" & strProp & "|")

End Function

Private Function IsFrxListLine(ByVal sString As String, _
                               strProp As String) As Boolean

  strProp = Split(Trim$(sString))(0)
  IsFrxListLine = InStr("|List|ItemData|", "|" & strProp & "|")

End Function

Private Function IsFrxTextLine(ByVal sString As String, _
                               strProp As String) As Boolean

  strProp = Split(Trim$(sString))(0)
  'note 'TextRTF' not yet extracted because I haven't worked it out
  IsFrxTextLine = InStr("|Text|Caption|Tag|ToolTipText|", "|" & strProp & "|")
...
Private Sub lstTextExtracted_Click()
  ...
  strFile = lstTextExtracted.List(lstTextExtracted.ListIndex)
  Beep
  strProp = Left$(strFile, InStr(strFile, ".") - 1)
  strProp = Mid$(strProp, InStrRev(strProp, "_") + 1)
  FN = FreeFile
  lstDisplay.Clear
  txtDisplay.Text = ""
  Select Case strProp
   Case "Tag", "Text", "ToolTipText", "Caption"
    Open strFile For Input As #FN
    strIn = Input$(LOF(1), #FN)
    Close #FN
    txtDisplay.Text = strIn
   Case "List", "ItemData"
    Open strFile For Input As #FN
    strIn = Input$(LOF(1), #FN)
    Close #FN
...
Private Sub mnuHelpPage_Click(Index As Integer)
  ...
  Select Case Index
   Case 0
    MsgBox "Why?" & vbNewLine & _
       ...
       "3. Embedded Text and Lists are difficult to edit."
   Case 1
    MsgBox "How?" & vbNewLine & _
       "1. Select the binary files you want (*.frx *.dox *.ctx *.dsx *pax). CommonDialog allows Multiselect" & vbNewLine & _
       "2. Click 'Start Extraction' (You can set which  data to extract using the Checkboxes" & vbNewLine & _
       "3. All files are extracted to the Destination folder (Default = source folder)." & vbNewLine & _
       "4. FileName format is 'FileName_ControlName_Property.FileExtention' to simplify reloading." & vbNewLine & _
       "   ControlName includes Index."
   Case 2
    MsgBox "Graphics details" & vbNewLine & _
       "1. The program extracts ALL recognised graphics(bmp, gif, ico, jpg, wmf (cur as ico, dib as bmp, jpeg & Tiff as jpg)" & vbNewLine & _
       "2. Imaglist Property is 'ListImage#'. Key and Tag values are included in brackets." & vbNewLine & _
   ...
   Case 3
    MsgBox "Text/List details" & vbNewLine & _
       "1. The program extracts Text, Caption, Tag, ToolTipText, List and ItemData from all controls that have those properties and that are assigned a value from the VB IDE Property window" & vbNewLine & _
       "2. Optionally you can also extract the string values even if they are stored directly in the frm rather than the binary (less than 125 characters)" & vbNewLine & _
       "3. FileName format is 'FileName_ControlName_Property.TXT'." & vbNewLine & _
       "   ControlName includes Index." & vbNewLine & _
       "4. At present you will have to do some coding to move the data into your code." & vbNewLine & _
       "5. ItemData made up of zeros is automatically generated even if you didn't include it." & vbNewLine & _
       "6. Selecting images in the list to view the date in the textbox or list." & vbNewLine & _
       "7. You can delete the file in the main viewer." & vbNewLine & _
       vbNewLine & _
       "8. Still working on extracting TextRTF.", vbInformation, strBaseCaption
...
Private Sub ProcessOneImage(sImageData As String, _
  ...
  If LenB(strExt) Then
   ...
   Else
    ...
    sImageData = ""
  End If
  lblExtractVB(0).Caption = nImageCount & " image" & IIf(nImageCount = 1, vbNullString, "s") & " extracted"

End Sub

Private Sub ProcessOneString(sImageData As String, _
                             sFolder As String, _
                             strFormName As String, _
                             strControlName As String, _
                             strControlIndex As String, _
  ...
  If LenB(sImageData) Then
    ProgressBar imgThumbnail.Count / MaxPossible * 100
    'strExt = GetImageExtention(sImageData)
    strExt = "txt" 'GetImageExtention(sImageData)
    If LenB(strExt) Then
      sImageFile = AttachPath(strFormName & IIf(strFormName <> strControlName, "_" & strControlName, "_") & IIf(Len(strControlIndex), "(" & strControlIndex & ")", vbNullString) & "_" & strControlProperty & "." & strExt, sFolder)
      'safety back up(protects you from overwriting new images with older ones in the frx)
      If FileExist(sImageFile) Then
        File2BAK sImageFile
      End If
      SaveTheText sImageFile, sImageData
      lstTextExtracted.AddItem ExtractFileName(sImageFile)
      sImageData = ""
     Else
      sImageData = ""
    End If
  ...
  End If
  lblExtractVB(1).Caption = nTextCount & " Text/List" & IIf(nImageCount = 1, vbNullString, "s") & " extracted"
  cmdDeleteText.Enabled = lstTextExtracted.ListCount
...
Private Sub SaveTheText(ByVal strFile As String, _
                        ByVal strData As String)
...
':)Code Fixer V2.9.8 (15/02/2005 11:26:36 AM) 24 + 1264 = 1288 Lines Thanks Ulli for inspiration and lots of code.
